# Tapping functionality adapted from tapping_static experiment
import json
import os
import tempfile
import numpy as np
...
# Import the iterated tapping analysis components
try:
    ...
    ITERATED_TAPPING_AVAILABLE = True
except ImportError:
    ITERATED_TAPPING_AVAILABLE = False
    print("Warning: reppextension.iterated_tapping not available. Using basic REPP analysis.")
...
from psynet.page import PageMaker
...
# Configuration for iterated tapping analysis
if ITERATED_TAPPING_AVAILABLE:
    from repp.config import ConfigUpdater
    config_itap = ConfigUpdater.create_config(
        sms_tapping,
        {
    ...
    stimulus_itap = REPPStimulusItap("itap", config=config_itap)
    analysis_itap = REPPAnalysisItap(config=config_itap)
...
nodes_music = [
    StaticNode(
        definition={
            "stim_name": name,
            "audio_filename": os.path.join("static", "music", audio_file),
            "onset_filename": os.path.join("static", "music", onset_file),
        },
        assets={
...
def extract_tap_timings(audio_file: str, stimulus_info: dict, use_iterated_analysis: bool = False):
    """
    Extract tap timings from audio recording using REPP analysis.
    ...
    stimulus_info : dict
        Information about the stimulus (markers, onsets, etc.)
    use_iterated_analysis : bool
        Whether to use the more sophisticated iterated tapping analysis
        
    Returns:
    --------
    dict
        Dictionary containing tap timings and analysis results
    """
    if use_iterated_analysis and ITERATED_TAPPING_AVAILABLE:
        # Use the iterated tapping analysis for more detailed tap extraction
        title_in_graph = "Tap Timing Analysis"
        
        # Create a dummy random seed for compatibility (not used in basic analysis)
        dummy_seed = [500.0] * 3  # 500ms intervals
        ...
        output, analysis, is_failed, output_iteration = analysis_itap.do_analysis(
            ...
            audio_file,
            title_in_graph,
            "tap_analysis_plot.png"
        )
        
        # Extract tap timings from the analysis
        tap_timings = {
            "failed": is_failed["failed"],
            "reason": is_failed["reason"],
            "tap_onsets": output_iteration.get("resp_onsets_complete", []),
            "inter_tap_intervals": output_iteration.get("resp_iois", []),
            "analysis_output": json.dumps(output, cls=NumpySerializer),
            "detailed_analysis": json.dumps(analysis, cls=NumpySerializer),
            "iteration_output": json.dumps(output_iteration, cls=NumpySerializer),
        }
        
    else:
        # Use basic REPP analysis
        title_in_graph = "Tap Timing Analysis"
        analysis = REPPAnalysis(config=sms_tapping)
        output, analysis, is_failed = analysis.do_analysis(
            stimulus_info, audio_file, title_in_graph, "tap_analysis_plot.png"
        )
        
        # Extract basic tap information
        tap_timings = {
            "failed": is_failed["failed"],
            "reason": is_failed["reason"],
            "analysis_output": json.dumps(output, cls=NumpySerializer),
        ...
        # Try to extract tap onsets from the analysis output
        if "resp_onsets" in output:
            tap_timings["tap_onsets"] = [as_native_type(x) for x in output["resp_onsets"]]
        if "resp_iois" in output:
            tap_timings["inter_tap_intervals"] = [as_native_type(x) for x in output["resp_iois"]]
...
class TapTrialAnalysis(AudioRecordTrial, StaticTrial):
    ...
    def extract_tap_timings(self, audio_file: str, use_iterated_analysis: bool = False):
        """
        Extract detailed tap timings from the recording.
        ...
        """
        info = self.get_info()
        return extract_tap_timings(audio_file, info, use_iterated_analysis)
...
class TapTrialISO(TapTrial):
    ...
    def get_bot_response_media(self):
        return {
            "iso_800ms": "static/example_iso_slow_tap.wav",
            "iso_600ms": "static/example_iso_fast_tap.wav",
...
class TapTrialMusic(TapTrial):
    ...
    def get_bot_response_media(self):
        return {
            "track1": "static/example_music_tapping_track_1.wav",
            "track2": "static/example_music_tapping_track_7.wav",
        }[self.definition["stim_name"]]
    ...
    return join(
        InfoPage(
            Markup(
                ...
                <li>At the beginning and end of each rhythm, you will hear three consecutive beeps.
        ...
        StaticTrialMaker(
            ...
            max_trials_per_participant=len(nodes_iso),
...
def music_tapping():
    return join(
        ...
        StaticTrialMaker(
            ...
            max_trials_per_participant=len(nodes_music),
            allow_repeated_nodes=False,
        ),
    )


def tapping_calibration():
    """Return the tapping calibration components"""
    return join(
        CustomREPPVolumeCalibrationMusic(),  # calibrate volume with music
...
def tapping_tasks():
    """Return the complete tapping tasks"""
    return join(
        tapping_calibration(),
        ISO_tapping(),
        music_tapping(),
...
# Custom REPPVolumeCalibrationMusic with modified introduction
class CustomREPPVolumeCalibrationMusic(REPPVolumeCalibrationMusic):
    @property
            ...
            lambda assets: InfoPage(
                Markup(
                    f"""
                      <h3>Rhythm test</h3>
                      <hr>
                      <b>During this test, you can ONLY use the laptop speakers and you need to be in a silent environment.
                      <br><br>
